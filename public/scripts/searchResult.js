    ...
    $('#searchbtn').click(function () {
        $.get("/getJobs", null, function (foundJobs) {
            ...
            foundJobs.forEach(job => {
                ...
                tr = tr + "<td scope='row'><button class='customBtn btn btn-primary' type='button' data-toggle='modal' data-target='#bidModal' data-jobid=" + job.id + ">Afgiv Bud</button></td>"
                tr = tr + "</tr></tbody>"
...
$('#bidModal').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    modal.find('.modal-title').text('New message to ' + jobid)
